// Copyright (c) 2009-2010 Satoshi NakamotoW
...
#if defined(NDEBUG)
# error "Pesacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x598223b47975ba9c5cf82c8f8954eacdd9f03c3a28ea2e03caf570d5a4556049");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Pesacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Pesacoin Signed Message:\n";
...
{
    // Pesacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 84000000); // Pesacoin: 84000k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Pesacoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Pesacoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Pesacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Pesacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x598223b47975ba9c5cf82c8f8954eacdd9f03c3a28ea2e03caf570d5a4556049");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is PesaCoin Cryptocurrency";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1492872316;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1894370;
        ...
        assert(block.hashMerkleRoot == uint256("0x412894a9a1672286e59c48bd54a317bff8ded70d45cc1f4919b4de17084c2e21"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Pesacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PesacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PesacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PesacoinMiner(CWallet *pwallet)
{
    printf("PesacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pesacoin-miner");
    ...
    try { loop {
        ...
        printf("Running PesacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PesacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PesacoinMiner, pwallet));
}
